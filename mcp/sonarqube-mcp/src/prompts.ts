 ...
 *
 * Prompts here are pure text templates — they do **not** call the SonarQube API
 * themselves. Each one instructs Claude to drive the workflow using the tools
 * defined in `tools.ts`, which keeps the prompt cheap to render, keeps
 * data-fetching auditable (every SonarQube call shows up as a distinct tool
 * call in the transcript), and matches how MCP prompts are meant to be used: as
 * guidance for the calling LLM, not as a place to hide side effects.
 */

import type { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { z } from 'zod';

/** Wrap prompt text in the single-user-message shape the MCP spec expects. */
function userMessage(text: string) {
  return { messages: [{ role: 'user' as const, content: { type: 'text' as const, text } }] };
}

/** Register every prompt on `server`. */
export function registerPrompts(server: McpServer): void {
  server.registerPrompt(
    'sonar_review',
    {
      title: 'SonarQube project review',
      description:
      ...
      argsSchema: { project_key: z.string().describe('The SonarQube project key to review.') },
    },
    ({ project_key }) =>
      userMessage(
...
specific issue key.`,
  ...
  server.registerPrompt(
    'pr_review',
    {
      ...
      argsSchema: {
        base_ref: z.string().optional().describe('Base git ref to compare against (default main).'),
        head_ref: z.string().optional().describe('Head git ref being reviewed (default HEAD).'),
      },
    },
    ({ base_ref, head_ref }) => {
      const base = base_ref ?? 'main';
      const head = head_ref ?? 'HEAD';
      return userMessage(
        `You are reviewing the code changes between \`${base}\` and \`${head}\` in
this repository, as a senior engineer would before approving a pull request.

1. Review changed code
   - Determine which files changed between \`${base}\` and \`${head}\`
     (e.g. via \`git diff --name-only ${base}...${head}\` through your
...
\`fix-sonarqube-issues\` / \`pre-pr-review\` skills instead.`,
  ...
  server.registerPrompt(
    'fix_issue',
    {
      ...
      argsSchema: { issue_key: z.string().describe('The SonarQube issue key to fix.') },
    },
    ({ issue_key }) =>
      userMessage(
