...
```
.
├── .mcp.json                       # Project-scoped server config, for developing this repo
├── ONBOARDING.md                   # Start here if you are new to the team
├── .claude-plugin/
│   └── marketplace.json             # Makes this repo installable as a plugin marketplace
├── plugin/                          # What teammates install
│   ├── .claude-plugin/plugin.json
│   ├── .mcp.json                    # Launches the bundle via ${CLAUDE_PLUGIN_ROOT}
│   ├── server/sonarqube-mcp.mjs     # Self-contained bundle (generated by `npm run build`)
│   └── skills/
...
## Installing as a plugin (recommended for users)

If you only want to *use* the server, install it as a Claude Code plugin — no clone,
no `npm`, no build. From any Claude Code session:
...
The server looks for `.env` in three places and uses the first that exists:

1. the **current working directory** — the project root when Claude Code launches it,
   so one project can override the global config;
2. **this package's own directory** — why `mcp/sonarqube-mcp/.env` works even though
   Claude Code starts the server from the repo root;
3. **`~/.config/sonarqube-mcp/.env`** — the global location, used by the plugin.

### Global configuration

When installed as a plugin the server lives in Claude Code's plugin cache, where
neither of the first two paths is somewhere you would think to edit. Put your
credentials in `~/.config/sonarqube-mcp/.env` instead — it applies to every project:
...
From a clone, `npm run setup -- --global --url=... --token=...` writes exactly that
file. `XDG_CONFIG_HOME` is honoured if set.
...
npm run build     # compile to dist/ AND regenerate the plugin bundle
npm run dev       # tsc --watch
```

`npm run build` runs `tsc` and then `esbuild`, so `plugin/server/sonarqube-mcp.mjs`
is refreshed every time. That file is **committed** — the plugin has no `node_modules`
to install from, so the bundle is the shipped artifact. Always commit it alongside the
`src/` change that produced it, or plugin users will run stale code.
